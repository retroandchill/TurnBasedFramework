...
public sealed class TargetTypeJsonSerializer(
    IConfigOptions<JsonSerializerOptions> jsonSerializerOptions
) : IGameDataEntrySerializer<UTargetType>
{
    ...
    public string SerializeData(IEnumerable<UTargetType> entries)
    {
        return JsonSerializer.Serialize(
            entries.Select(x => x.ToTargetTypeInfo()),
    ...
    public IEnumerable<UTargetType> DeserializeData(string source, UObject outer)
    {
        return JsonSerializer
            .Deserialize<TargetTypeInfo[]>(source, _jsonSerializerOptions)!
