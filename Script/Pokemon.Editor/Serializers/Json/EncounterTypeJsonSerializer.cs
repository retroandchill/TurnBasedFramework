...
public sealed class EncounterTypeJsonSerializer(IConfigOptions<JsonSerializerOptions> jsonSerializerOptions) : IGameDataEntrySerializer<UEncounterType>
{
    ...
    public string SerializeData(IEnumerable<UEncounterType> entries)
    {
        return JsonSerializer.Serialize(entries.Select(x => x.ToEncounterTypeInfo()), _jsonSerializerOptions);
    }

    public IEnumerable<UEncounterType> DeserializeData(string source, UObject outer)
    {
