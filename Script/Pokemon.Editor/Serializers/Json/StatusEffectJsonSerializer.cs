...
public sealed class StatusEffectJsonSerializer(
    IConfigOptions<JsonSerializerOptions> jsonSerializerOptions
) : IGameDataEntrySerializer<UStatusEffect>
{
    ...
    public string SerializeData(IEnumerable<UStatusEffect> entries)
    {
        return JsonSerializer.Serialize(
            entries.Select(x => x.ToStatusEffectInfo()),
    ...
    public IEnumerable<UStatusEffect> DeserializeData(string source, UObject outer)
    {
        return JsonSerializer
            .Deserialize<StatusEffectInfo[]>(source, _jsonSerializerOptions)!
