...
public sealed class BaseStatsConverter : PbsConverterBase<IReadOnlyDictionary<FGameplayTag, int>>
{
    ...
    public override string WriteCsvValue(IReadOnlyDictionary<FGameplayTag, int> value, PbsScalarDescriptor schema, string? sectionName)
    {
        return string.Join(",", _statOrder.Select(x => value.GetValueOrDefault(x, 1)));
    }

    public override IReadOnlyDictionary<FGameplayTag, int> GetCsvValue(string input, PbsScalarDescriptor scalarDescriptor, string? sectionName)
    {
        ...
        return _statOrder.Zip(values, (x, y) => (Stat: x, Value: y))
