...
public static partial class SpeciesMapper
{
    ...
    [MapProperty(
        ...
        nameof(SpeciesInitializer.EvolutionConditionInitializers),
        Use = nameof(ToEvolutionConditionInitializers)
    )]
    [MapProperty(
        nameof(SpeciesInfo.EvYield),
        nameof(SpeciesInitializer.EvYield),
        Use = nameof(MapEvYieldList)
    )]
    private static partial SpeciesInitializer ToSpeciesInitializer(
        this SpeciesInfo species,
        UObject? outer = null
    );
    ...
    private static FText? ToNullableText(this Option<FText> value) =>
        value.Match<FText?>(v => v, () => null);

    private static Option<FText> ToTextOption(this FText? value) => value ?? Option<FText>.None;

    private static EvolutionConditionInfo ToEvolutionConditionInfo(
        this FEvolutionCondition condition
    )
    {
    ...
    )
    {
        return conditions.Select(condition =>
            (Func<UObject, FEvolutionCondition>)(y => condition.ToEvolutionCondition(y))
        );
    }

    private static FEvolutionCondition ToEvolutionCondition(
        this EvolutionConditionInfo condition,
        UObject? outer = null
    )
    {
    ...
    private static IReadOnlyDictionary<FGameplayTag, int> MapEvYieldList(
    ...
    private static IReadOnlyList<EvYield> MapEvYieldDictionary(
        IReadOnlyDictionary<FGameplayTag, int> evYields
    )
    {
