...
public static class StructPropertyUtilities {

  private static readonly Regex BackingFieldRegex = new("<([a-zA-Z$_][a-zA-Z0-9$_]*)>k__BackingField");
  
  public static StructPropertyInfo GetPropertyInfo(this IFieldSymbol fieldSymbol) 
  {
    var backingFieldMatch = BackingFieldRegex.Match(fieldSymbol.Name);
    var propertyName = backingFieldMatch.Success ? backingFieldMatch.Groups[1].Value : fieldSymbol.Name;

    return new StructPropertyInfo(fieldSymbol.Type, propertyName) {
        MarshallerInfo = fieldSymbol.Type.GetMarshallerName()
    };
  }
  ...
  private static MarshallerInfo GetMarshallerName(this ITypeSymbol typeSymbol) 
  {
    switch (typeSymbol.SpecialType) {
      ...
      case SpecialType.System_Byte or SpecialType.System_SByte or SpecialType.System_Int32 or SpecialType.System_UInt32 or SpecialType.System_Int64 or SpecialType.System_UInt64 or SpecialType.System_Single or SpecialType.System_Double:
      ...
      default: {
        ...
        if (typeSymbol.MetadataName == "TMulticastDelegate`1") {
          return new MarshallerInfo($"MulticastDelegateMarshaller<{typeSymbol.ToDisplayString()}>");
        }
        
        if (typeSymbol.MetadataName == "TDelegate`1") {
          return new MarshallerInfo($"SingleDelegateMarshaller<{typeSymbol.ToDisplayString()}>");
        }

        var structAttribute = typeSymbol.GetAttributes()
        ...
        if (structAttribute is null) {
          return typeSymbol.IsUnmanagedType ? new MarshallerInfo($"UnmanagedTypeMarshaller<{typeSymbol.ToDisplayString()}>") : new MarshallerInfo($"ManagedObjectMarshaller<{typeSymbol.ToDisplayString()}>");
        }
