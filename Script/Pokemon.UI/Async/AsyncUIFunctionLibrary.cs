...
public class UAsyncUIFunctionLibrary : UBlueprintFunctionLibrary
{
    ...
    public static Task DisplayMessage(UObject worldContext, FText message, CancellationToken cancellationToken = default)
    {
        UWorldContextExtensions.WorldContext = worldContext;
        return IDisplayService.Instance.DisplayMessage(message, cancellationToken);
    }
}