...
public class UType : UObject, IGameDataEntry
{
    ...
    public float GetEffectiveness([Categories(TagCategory)] FGameplayTag otherType)
    {
        if (!otherType.IsValid)
            return Effectiveness.NormalMultiplier;

        if (Weaknesses.HasTag(otherType))
            return Effectiveness.SuperEffectiveMultiplier;
        if (Resistances.HasTag(otherType))
            return Effectiveness.NotVeryEffectiveMultiplier;

        return Immunities.HasTag(otherType)
            ? Effectiveness.IneffectiveMultiplier
            : Effectiveness.NormalMultiplier;
    }
}
...
public static class Effectiveness
{
    ...
    public static bool IsIneffective_type(
        this FGameplayTag attackType,
        params ReadOnlySpan<FGameplayTag> defendTypes
    )
    {
    ...
    public static bool IsNotVeryEffectiveType(
        this FGameplayTag attackType,
        params ReadOnlySpan<FGameplayTag> defendTypes
    )
    {
    ...
    public static bool IsResistantType(
        this FGameplayTag attackType,
        params ReadOnlySpan<FGameplayTag> defendTypes
    )
    {
    ...
    public static bool IsNormalType(
        this FGameplayTag attackType,
        params ReadOnlySpan<FGameplayTag> defendTypes
    )
    {
    ...
    public static bool IsSuperEffectiveType(
        this FGameplayTag attackType,
        params ReadOnlySpan<FGameplayTag> defendTypes
    )
    {
