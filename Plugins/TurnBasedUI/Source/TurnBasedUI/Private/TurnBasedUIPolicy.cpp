...
UTurnBasedUIPolicy* UTurnBasedUIPolicy::GetInstance(const UObject* WorldContextObject)
{
    return TOptionalPtr(GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
        ...
        .Map([](const UGameInstance* GameInstance) { return GameInstance->GetSubsystem<UTurnBasedUIManagerSubsystem>(); })
        .Map([](UTurnBasedUIManagerSubsystem* Subsystem) { return Subsystem->GetCurrentUIPolicy(); })
        .Get();
}

UWorld* UTurnBasedUIPolicy::GetWorld() const
{
    return GetOwner()->GetGameInstance()->GetWorld();
}

UTurnBasedUIManagerSubsystem* UTurnBasedUIPolicy::GetOwner() const
{
    return CastChecked<UTurnBasedUIManagerSubsystem>(GetOuter());
}

UPrimaryGameLayout* UTurnBasedUIPolicy::GetRootLayout(const ULocalPlayer* Player) const
{
    const auto LayoutInfo = RootViewportLayouts.FindByKey(Player);
    return LayoutInfo != nullptr ? LayoutInfo->RootLayout : nullptr;
}

void UTurnBasedUIPolicy::RequestPrimaryControl(UPrimaryGameLayout* Layout)
{
...
void UTurnBasedUIPolicy::AddLayoutToViewport(ULocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
    UE_LOG(LogTurnBasedUI, Log, TEXT("[%s] is adding player [%s]'s root layout [%s] to the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));
...
void UTurnBasedUIPolicy::RemoveLayoutFromViewport(ULocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
    ...
    UE_LOG(LogTurnBasedUI, Log, TEXT("[%s] is removing player [%s]'s root layout [%s] from the viewport"), *GetName(), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));

    Layout->RemoveFromParent();
    if (LayoutSlateWidget.IsValid())
    {
        UE_LOG(LogTurnBasedUI, Log, TEXT("Player [%s]'s root layout [%s] has been removed from the viewport, but other references to its underlying Slate widget still exist. Noting in case we leak it."), *GetNameSafe(LocalPlayer), *GetNameSafe(Layout));
    }

    OnRootLayoutRemovedFromViewport(LocalPlayer, Layout);
}

void UTurnBasedUIPolicy::OnRootLayoutAddedToViewport_Implementation(ULocalPlayer* LocalPlayer,
...
void UTurnBasedUIPolicy::OnRootLayoutRemovedFromViewport_Implementation(ULocalPlayer* LocalPlayer,
    UPrimaryGameLayout* Layout)
{
    // No implementation in this class
}

void UTurnBasedUIPolicy::OnRootLayoutReleased_Implementation(ULocalPlayer* LocalPlayer, UPrimaryGameLayout* Layout)
{
}

void UTurnBasedUIPolicy::CreateLayoutWidget(ULocalPlayer* LocalPlayer)
{
...
TSubclassOf<UPrimaryGameLayout> UTurnBasedUIPolicy::GetLayoutWidgetClass() const
{
    return LayoutClass.LoadSynchronous();
}

void UTurnBasedUIPolicy::NotifyPlayerAdded(ULocalPlayer* LocalPlayer)
{
...
void UTurnBasedUIPolicy::NotifyPlayerDestroyed(ULocalPlayer* LocalPlayer)
{
    NotifyPlayerRemoved(LocalPlayer);
