...
UPrimaryGameLayout* UPrimaryGameLayout::GetInstance(ULocalPlayer* LocalPlayer)
{
    return TOptionalPtr(LocalPlayer)
        ...
        .Map(&UTurnBasedUIPolicy::GetRootLayout, LocalPlayer)
        .Get();
}
...
void UPrimaryGameLayout::SetIsDormant(bool bNewIsDormant)
{
    if (bIsDormant != bNewIsDormant)
    {
        ...
        UE_LOG(LogTurnBasedUI, Display, TEXT("%s PrimaryGameLayout Dormancy changed for [%d] from [%s] to [%s]"), PrimaryPlayerStr, PlayerId, OldDormancyStr, NewDormancyStr);
                                                    ...
                                                    bool bIsTransitioning)
{
    if (bIsTransitioning)
    {
        const FName SuspendToken = UTurnBasedUIExtensions::SuspendInputForPlayer(GetOwningLocalPlayer(), TEXT("GlobalStackTransion"));
    ...
    else
    {
        if (ensure(SuspendInputTokens.Num() > 0))
        {
            ...
            UTurnBasedUIExtensions::ResumeInputForPlayer(GetOwningLocalPlayer(), SuspendToken);
        }
    }
}
